...
static PG_GLOBAL_RUNTIME sem_t **mySemPointers;	/* keep track of created semaphores */
#else
static PG_GLOBAL_SHMEM PGSemaphore sharedSemas; /* array of PGSemaphoreData in shared memory */
#endif
static PG_GLOBAL_RUNTIME int numSems;	/* number of semas acquired so far */
static PG_GLOBAL_RUNTIME int maxSems;	/* allocated size of above arrays */
static PG_GLOBAL_RUNTIME int nextSemKey;	/* next name to try */
