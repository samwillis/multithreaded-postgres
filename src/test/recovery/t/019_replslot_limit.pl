...
my $threaded_primary3 =
  $node_primary3->safe_psql('postgres', 'SHOW multithreaded') eq 'on';

SKIP:
{
	skip "SIGSTOP/SIGCONT target OS processes, not threaded backend IDs", 2
	  if $threaded_primary3;
...
my $threaded_primary4 =
  $primary4->safe_psql('postgres', 'SHOW multithreaded') eq 'on';

SKIP:
{
	skip "threaded logical subscriber shutdown is tracked by the subscription blocker", 2
	  if $threaded_primary4;
